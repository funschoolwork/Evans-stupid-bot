...
GUILD_ID          = 1476431914851369044      # Only server the bot works in
DOWNLOAD_CHANNEL  = 1476431915870322834      # Only channel .download and .setfile work in
OWNER_ID          = 1424768569710739619      # User who can use .setfile
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        return  # silently fail wrong guild / channel / perms
    if isinstance(error, commands.MissingRequiredArgument):
        p = settings["prefix"]
        await ctx.send(f"⚠️ Missing argument. Try `{p}help`", delete_after=8)
...
async def setfile(ctx, *, custom_message: str = None):
    """
    [OWNER] Attach a file to this command to set it as the installer.
    Optionally include a custom message.

    Usage:
      .setfile                          (attach a file)
      .setfile Here is the new build!   (attach a file + custom message)
    """
    if not ctx.message.attachments:
        await ctx.send(
            "❌ Please **attach a file** to your message when using `.setfile`.\n"
            "Example: Upload the installer file and type `.setfile Here is v2.0!`",
            delete_after=12
        )
        return
    ...
    settings["installer_message"] = custom_message if custom_message else "Here is the latest installer!"
    ...
    await ctx.send(
        ...
        f"Anyone who runs `.download` in this channel will now receive it."
    )
...
async def download_file(ctx):
    """
    Sends the installer file set by the owner.
    ...
    if not file_path or not os.path.exists(file_path):
        await ctx.reply(
            ...
            f"An admin must use `.setfile` (with a file attached) to set one.",
            delete_after=10
        )
        return
